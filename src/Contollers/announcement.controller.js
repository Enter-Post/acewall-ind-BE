...
export const createAnnouncement = asyncHandler(async (req, res) => {
  ...
  for (const file of files) {
    const result = await uploadToCloudinary(file.buffer, "announcement_files");
    attachments.push({
  ...
  try {
    await notifyNewAnnouncement(courseId, course.courseTitle, title, teacherId);
  } catch (error) {
...
export const getAnnouncementsForStudent = asyncHandler(async (req, res) => {
  ...
  const announcements = await Announcement.find({
    ...
    .select(
      "title message course teacher attachments links createdAt updatedAt",
    )
    .sort({ createdAt: -1 });
